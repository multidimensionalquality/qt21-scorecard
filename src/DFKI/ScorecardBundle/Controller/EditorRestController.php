...
class EditorRestController extends Controller {
	...
	public function postIssueAction() {
		...
		if (empty ( $segmentid ) || empty ( $issueid ) || empty ( $severity ) || empty ( $side ) || empty ( $tempReportId ) || ! ($severity == "minor" || $severity == "critical" || $severity == "major") || ! ($side == "source" || $side = "target")) {
		...
		$issueProjectMapping = $em->getRepository ( "DFKIScorecardBundle:IssueProjectMapping" )->findOneBy ( array (
	...
	/**
	 * helper function.
	 * serialize import fields from issue report
	 *
	...
	public function postNotesAction() {
		...
		if (empty ( $segmentid ) || empty ( $notes )) {
	...
	/**
	 * delete issue report.
	 * expects post variable "highlights" and "side"
	 *
	...
	public function postHighlightsAction($segmentid) {
		...
		if ($highlights == null || ! ($side == "source" || $side == "target")) {
			throw new BadRequestHttpException ();
		}
