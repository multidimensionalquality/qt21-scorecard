...
class EditorService{
	...
	public function getIssueReports(Project $project){
		$issueReports = $this->em->createQuery(
			"SELECT ir FROM DFKIScorecardBundle:IssueReport ir, DFKIScorecardBundle:Segment s 
			WHERE s.project = :project AND ir.segment=s.id"
	...
	/**
	 * calculate score of project. returns array with fields "sourceScore", "targetScore", "compositeScore"
	 * 
	...
	public function getProjectScore($project){
		...
		for( $i=0; $i<sizeof($reports); $i++ ){
			...
			$severity = 0;
			if( $report->getSeverity() == "minor" ) $severity = 1;
			else if( $report->getSeverity() == "major" ) $severity = 10;
			else if( $report->getSeverity() == "critical" ) $severity = 100;
			else throw new \Exception();
			
			$severity *= $report->getIssueProjectMapping()->getWeight();
			
			if( $report->getSide() == "source" ) $sourcePenalty += $severity;
			else $targetPenalty += $severity;
		}
